...
def ping(host, ping_count) -> CheckResult:
    host = icmplib.ping(host, privileged=False,count=ping_count)
    if host.is_alive:
        if host.packet_loss > 0.0:
            return Warn(f"ICMP '{host}' ({host.address}) unreliable! packet loss {host.packet_loss*100}%")
        return Ok(f"ICMP '{host}' reachable ({host.avg_rtt}ms)")
